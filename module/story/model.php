...
class storyModel extends model
{
    ...
    public function create($executionID = 0, $bugID = 0, $from = '', $extra = '')
    {
        ...
        if(!dao::isError())
        {
            ...
            if($bugID > 0)
            {
                ...
                if(($this->config->edition == 'biz' || $this->config->edition == 'max' || $this->config->edition == 'open') && !dao::isError() && $oldBug->feedback) $this->loadModel('feedback')->updateStatus('bug', $oldBug->feedback, 'closed', $oldBug->status);
    ...
    public function updateParentStatus($storyID, $parentID = 0, $createAction = true)
    {
        ...
        if($status and $oldParentStory->status != $status)
        {
            ...
            if(!dao::isError())
            {
                ...
                // if增加 $this->config->edition == 'open' chenjj 230115
                if(($this->config->edition == 'biz' || $this->config->edition == 'max' || $this->config->edition == 'open') && $oldParentStory->feedback) $this->loadModel('feedback')->updateStatus('story', $oldParentStory->feedback, $newParentStory->status, $oldParentStory->status);
            }
        }
        else
        {
        ...
        if(!empty($storyIdList))
        {
            ...
            foreach($stories as $storyID => $story)
            {
                ...
                if(!dao::isError())
                {
                    ...
                    // if增加 $this->config->edition == 'open' chenjj 230115
                    if(($this->config->edition == 'biz' || $this->config->edition == 'max' || $this->config->edition == 'open') && $oldStory->feedback && !isset($feedbacks[$oldStory->feedback]))
                    {
     ...
     * @return void
     */
    public function getParentStoryPairs($productID, $append = '', $executionID=-1)
    {
        if($executionID > 0){
...
//        ChromePhp::log($stories);
    ...
    public function printCell($col, $story, $users, $branches, $storyStages, $modulePairs = array(), $storyTasks = array(), $storyBugs = array(), $storyCases = array(), $mode = 'datatable', $storyType = 'story', $execution = '', $isShowBranch = '')
    {
        ...
        $canView     = common::hasPriv($story->type=='taskPoint'?'story':$story->type, 'view', null, "storyType=$story->type");
